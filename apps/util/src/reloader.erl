...
%% modified to use skip_timer to avoid CPU spike after OS hibernation wakeup
...
init([]) ->
    {ok, TRef} = skip_timer:send_interval(skip_timer:seconds(1), doit),
...
terminate(_Reason, State) ->
    {ok, cancel} = skip_timer:cancel(State#state.tref),
    ok.
