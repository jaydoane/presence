...
%% skip modification from http://www.couchbase.com/issues/browse/MB-3321
%% specifically, http://review.couchbase.org/#/c/25551/
...
-define(TIMER_TAB, skip_timer_tab).
-define(INTERVAL_TAB, skip_timer_interval_tab).

-define(SERVER, skip_timer_server).
...
timer_timeout(SysTime) ->
	...
	Key ->
		...
		[{{Time, Ref}, Repeat = {repeat, Interv, To}, MFA} = TimerEntry] ->
		    ets:delete(?TIMER_TAB,Key),
		    NewTimeCandidate = Time + Interv,
                    NewTime = new_time_maybe_skipping_forward(NewTimeCandidate,
                                                              SysTime, Interv, TimerEntry),
